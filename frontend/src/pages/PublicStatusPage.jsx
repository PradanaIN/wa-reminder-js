...
export default function PublicStatusPage() {
  ...
  return (
    <PublicLayout>
      ...
      <section
        id="public-activity"
        className="mt-12 grid gap-8 lg:grid-cols-[2fr,1fr]"
      >
        <Card className="space-y-6 border-white/10 bg-slate-900/65">
          ...
          {logsLoading ? (
            ...
            <pre className="max-h-72 overflow-y-auto rounded-xl border border-white/10 bg-slate-950/70 p-4 text-xs text-slate-300">
              {logsData.logs.join("\n")}
            </pre>
          ) : (
            <DataPlaceholder
